...
class PipelineConfig:
    ...
    # 난이도 설정
    difficulties: Dict = field(default_factory=lambda: {
        "easy": {
        ...
        "extreme": {
            "noise_types": ["unrelated", "english", "similar", "contradictory", "partial"],
...
class InferenceBenchmarkPipeline:
    ...
    def prepare_english_contexts(self, base_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(df) == len(base_df):
            ...
            df.to_csv(self.output_path / 'shuffled_english_contexts.csv', index_label='id')
    ...
    def prepare_noise_contexts(self, base_df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        noise_dfs['similar'] = self._load_or_create('similar_domain_contexts.csv', create_similar)
        ...
        noise_dfs['contradictory'] = self._load_or_create('contradictory_contexts.csv', create_contradictory)
        ...
        noise_dfs['partial'] = self._load_or_create('partial_overlap_contexts.csv', create_partial)
        
        return noise_dfs
    
    def create_noisy_datasets(self, base_df: pd.DataFrame, english_df: pd.DataFrame, 
                             noise_dfs: Dict[str, pd.DataFrame]) -> List[pd.DataFrame]:
        """노이즈 조합 데이터셋 생성"""
        print("\n" + "=" * 80)
                ...
                for idx, row in base_df.iterrows():
                    ...
                    for noise_type in difficulty_config['noise_types']:
                        if noise_type == 'english':
                            ...
                            noise_idx = random.randint(0, len(noise_dfs[noise_type]) - 1)
                            noises.append(noise_dfs[noise_type].iloc[noise_idx]['context'])
                    
                    # 노이즈 배치
                    noisy_context = self.positioner.apply(row['context'], noises, position)
    ...
    def create_summary(self, datasets: List[pd.DataFrame]) -> pd.DataFrame:
        ...
        for df in datasets:
            ...
            rows.append({
                ...
                'noise_ratio': df['noisy_context'].str.len().mean() / df['original_context'].str.len().mean()
            })
