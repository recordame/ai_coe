...
def main(args):
    ...
    with open(output_filename, "a", encoding="utf-8") as f:
        for batch in tqdm(
            ...
            for headline, article_text, qa_list_json in zip(
                batch["Headline"], batch["Article"], batch["qa_list"]
            ):
                qa_list = json.loads(qa_list_json)
                new_qa_list = [qa for qa in qa_list]
                prompts = batch_chat_template(
                    article_text, new_qa_list, tokenizer, args
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt_type", type=str, default="answer_generation", help="lang"
    )
