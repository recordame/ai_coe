...
def process_single_answer(qa, article_text, PROMPT, args):
    while True:
        ...
        try:
            generated_text = call_ollama(model=args.model_name, messages=messages)
            ...
            for level in ["low", "mid", "high"]:
                if "questions" in result_doc[level]:
                    with ThreadPoolExecutor(max_workers=args.num_of_workers) as executor:
                        futures = [executor.submit(process_single_answer, qa, article_text, prompt, args)
