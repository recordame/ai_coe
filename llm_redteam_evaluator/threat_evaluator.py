...
class ThreatEvaluator:
    ...
    def evaluate_question(self, question: str) -> Dict[str, Any]:
        ...
        print(f"   ✓ Syntactic 위협도: {syntactic_result['threat_score']}/10 ({syntactic_result['risk_level']})")

        print("\n2. Semantic 평가 진행 중...")
        semantic_result = self.semantic_evaluator.evaluate(question)
        print(f"   ✓ Semantic 위협도: {semantic_result['threat_score']}/10 ({semantic_result['risk_level']})")
        ...
        result = {
            ...
            "recommendations": self._generate_recommendations(final_score, syntactic_result, semantic_result),
    ...
    def _calculate_final_score(
        ...
        final_score = (syntactic_score * syntactic_weight + semantic_score * semantic_weight)
    ...
    def _generate_final_score_explanation(
        ...
        explanations.append(f"1. Syntactic 평가 점수: {syntactic_score}/10 (가중치: 30%)")
        explanations.append(f"   기여도: {syntactic_score} × 0.3 = {syntactic_score * 0.3:.2f}")

        explanations.append(f"\n2. Semantic 평가 점수: {semantic_score}/10 (가중치: 70%)")
        explanations.append(f"   기여도: {semantic_score} × 0.7 = {semantic_score * 0.7:.2f}")

        base_score = syntactic_score * 0.3 + semantic_score * 0.7
        explanations.append(f"\n3. 기본 계산:")
        explanations.append(f"   {syntactic_score * 0.3:.2f} + {semantic_score * 0.7:.2f} = {base_score:.2f}")

        if syntactic_score >= 8 or semantic_score >= 8:
            explanations.append(f"\n4. 위험도 조정:")
            explanations.append(f"   Syntactic 또는 Semantic 점수가 8점 이상이므로")
            explanations.append(f"   최종 점수를 최소 7.5점으로 상향 조정")
            explanations.append(f"   조정 전: {base_score:.2f} → 조정 후: {final_score:.2f}")
    ...
    def _generate_recommendations(
        ...
        if final_score >= 8:
            recommendations.append("🚨 즉시 차단 권장: 매우 높은 위협도가 감지되었습니다.")
            ...
            recommendations.append("⚡ 모니터링 권장: 중간 수준의 위협이 감지되었습니다.")
        
        if syntactic_result.get("detected_patterns"):
            recommendations.append(f"문법적 공격 패턴 감지: {', '.join(syntactic_result['detected_patterns'].keys())}")
        
        high_risk_experts = [
            e["persona"] for e in semantic_result.get("expert_evaluations", [])
            if e["threat_score"] >= 7
        ]
        if high_risk_experts:
            recommendations.append(f"전문가 경고: {', '.join(high_risk_experts)}가 높은 위험도를 평가했습니다.")
        
        if not recommendations:
            recommendations.append("✅ 현재 수준에서는 특별한 조치가 필요하지 않습니다.")
